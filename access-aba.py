import csv, requests, json, mygene, argparse
...
parser.add_argument('--filter', help='file of structure to search Allen Brain Atlas for')
args = parser.parse_args()

desired_structures = open(args.filter,'rb').read().splitlines()
...
total_iters = len(desired_structures)*len(genes_to_probes)
iter_count = 0
tries = 0
excepts = 0
with open('gene-expression-by-area-filtered-f.txt','wb') as outfile:
	for desired_structure in desired_structures:
		for structure_name,structure_id in structures.iteritems():
			if desired_structure in structure_name: #ABA name will be more detailed
				ap('Looking for %s'%structure_name)
				...
				for gene_id,probes in genes_to_probes.iteritems():
					try:
						...
						#ap('\t %.04f'%average_expression_level)					
...
json.dump(genes_by_area,open('gene-expression-by-area2-filtered-f.json','wb'))
